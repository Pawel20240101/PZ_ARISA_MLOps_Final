...
if __name__ == "__main__":
    ...
    if champ_mv is None:
        ...
        if chall_mv is None:
            ...
            logger.info(
                "Did not found champion or challenger, promoting newest model to champion."
            )
            client.set_registered_model_alias(MODEL_NAME, "champion", model_info.version)
        else:
            logger.info(
                "Found challenger model with no champion, promoting challenger to champion."
            )
    ...
    if champ_mv and chall_mv:
        ...
        if f1_chall >= f1_champ:
            logger.info(
                "Challenger model surpassed metric of current champion, "
            ...
            challenge_failed_exc = (
                "Challenger model does not surpass metric of current champion, "
                "ending predict workflow."
            )
